...
public:
  Chain(std::uint16_t nm = 3) { setNbOfMasses(nm); }

  ~Chain() { }

  std::uint16_t size() { return masses.size(); }
  ...
  std::vector<MassPtr>& getMasses() { return masses; }
  ...
  void computeState()
  {
    for(std::uint8_t d = 0; d < dimension + 1; ++d)
    {
      ...
      for(auto m : masses)
      {
        float val = (d < dimension) ? m->getPosition()[d] : m->getSpeedNorm();
  ...
  Chain::State& getState() { return state; }
  ...
  {
    return {masses[0]->getPosition(), masses[masses.size() - 1]->getPosition()};
  }
